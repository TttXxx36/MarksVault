...
- 快照列表显示容量预警；删除受保护快照必须经过用户明确确认，不会静默清理。
- 快照保留 Firefox separator 和 `unmodifiable` 元数据；恢复预览会将分隔线、受管理节点列为冲突并跳过。
...
- Chrome MV3、Firefox MV2、Edge MV3 构建通过；Firefox `web-ext lint`：0 errors、0 notices、2 个来自第三方 MUI bundle 的既有 `innerHTML` warning。Firefox 使用稳定 Gecko ID、数据声明和 MV2 `optional_permissions`。
- Jest 全量：18 suites / 157 tests passed；TypeScript 类型检查、ESLint（0 errors、23 个既有 warning）、manifest 校验和 SHA256 校验均通过。
