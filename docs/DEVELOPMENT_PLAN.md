...
截至 2026-08-26，v1.6.0、v2.0.0、v2.0.1、v2.1.0、v2.1.1 均已有独立标签、Release 和生产资产；本轮 v2.1.2 作为维护版本继续保留这些公开版本。三端构建、Manifest 检查和 Firefox lint 已纳入质量门槛。`docs/BASELINE.md` 记录了可复现命令和结果，README 大图已移出安装包。
...
### 18.7 v2.1.2：AI 延迟预算与大批量分类性能

本节记录本轮 Grill with Docs 确认的 AI 性能修复范围。真实 API 验证只使用用户在本机安全环境中配置的、已轮换的凭据和合成书签，不使用聊天中暴露的旧 Key。

#### 18.7.1 延迟边界与重试

- 单次 AI 请求默认超时 30 秒；一个分类批次默认总预算 90 秒；单次请求最多尝试 2 次。
- 旧配置中的 60 秒/20 条隐式默认值迁移到 v2.1.2 的 30 秒/10 条默认值；用户显式调整后的值保留在合法范围内。
- 批次预算耗尽后只标记当前批次失败，不能阻塞整个任务；失败批次仍可单独重试。
- 超时、限流、服务端错误和网络错误携带稳定错误码与实际尝试次数，认证和协议错误不自动重试。

#### 18.7.2 自适应批次与 100+ 书签

- 默认批量大小调整为 10；响应内容过大、请求超时或批次预算仍有余量时，将较大批次拆成更小子批次。
- 拆分复用原批次的截止时间，不得通过拆分绕过总预算；已完成子批次不得重复发送。
- 增加 100 条合成书签的 10 批基准测试，并保留 20 条和慢响应故障注入测试。
- 真实 API 烟测按 1/20/100 条合成书签分层执行，只记录耗时、批次数和错误码，不记录标题、URL、响应正文或 Key。

#### 18.7.3 后台进度心跳

- Popup 和任务页面每秒读取后台任务状态，显示当前批次、实际尝试次数和已耗时秒数。
- IndexedDB 检查点是进度的权威来源；`storage.onChanged` 只能作为兼容通知，不能作为唯一刷新机制。
- 关闭 Popup 不取消任务；浏览器重启后仍需用户明确继续，不自动重新发送 AI 请求。
